...
const TRANSCRIBE_TIMEOUT_BASE_MS = 90_000;
const TRANSCRIBE_TIMEOUT_MAX_MS = 600_000;

function transcribeTimeoutMs(byteLength: number): number {
  const extra = Math.floor(byteLength / 16_000) * 1000;
  return Math.min(TRANSCRIBE_TIMEOUT_MAX_MS, TRANSCRIBE_TIMEOUT_BASE_MS + extra);
}
...
async function transcribeTurnAudio(
  ...
  const timeout = setTimeout(
    () => controller.abort(),
    transcribeTimeoutMs(buffer.length)
  );
