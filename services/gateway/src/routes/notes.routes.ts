...
export async function notesRoutes(fastify: FastifyInstance) {
  ...
  fastify.delete('/by-message/:sourceMessageId', async (request, reply) => {
    try {
      const userId = requireUserId(request);
      const { sourceMessageId } = request.params as { sourceMessageId: string };
      const result = await deleteNoteBySourceMessageId(userId, sourceMessageId);
