...
async def _filter_models_async(
    model_ids: List[str],
    *,
    task: str,
...
async def _adaptive_candidates_async(
    model_ids: List[str],
    *,
    task: str,
    ...
    if _use_model_health_v2():
        from llm import ranking

        ranked_rows = await ranking.rank_models_for_task(model_ids, task)
        ranked_ids = [r["modelId"] for r in ranked_rows]
    else:
        ranked_ids = health_metrics.sort_models_by_latency(model_ids, provider_for_model, task=task)
    ...
    if cfg.get("adaptiveEnabled", True):
        hedged = False
        if _use_model_health_v2() and ranked_rows:
            if ranked_rows[0].get("state") == "degraded":
                hedged = True
        else:
            prov = provider_for_model(primary)
            hedged = health_metrics.should_race_primary(
                prov,
                task=task,
        ...
        if hedged:
            candidates = ranked_ids[: cfg["maxConcurrentPerTier"]]
            race_mode = "hedged"
...
async def iter_tier_race_tokens(
    ...
    *,
    ...
    candidates = await _adaptive_candidates_async(
        await _filter_models_async(
            model_ids, task=task, session_model_id=session_model_id, allow_degraded=True
        ),
        task=task,
        race_meta=race_meta,
        session_model_id=session_model_id,
        ...
        while pending:
            ...
            for finished in list(done):
                ...
                if isinstance(item, Exception):
                    ...
                    if _use_model_health_v2():
                        await model_health.record_request(
                            mid, task=task, latency_ms=0, success=False
                        )
                ...
                if winner is None:
                    ...
                    if _use_model_health_v2():
                        await model_health.record_request(
                            mid, task=task, latency_ms=latency_ms, success=True
                        )
