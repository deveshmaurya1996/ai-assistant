...
export interface CapabilitySourceEntry {
  id: string;
  domain: 'messaging' | 'email' | 'calendar' | 'files' | 'contacts' | 'platform' | 'image';
  description: string;
  risk: RiskLevel;
...
export const CAPABILITY_SOURCE: CapabilitySourceEntry[] = [
  ...
  },
  {
    id: 'image.edit',
    domain: 'image',
    description: "Generate or edit an image from a text prompt (optional source image)",
    risk: 'medium',
    requiresConfirmation: true,
    plannerVisible: true,
    ...
    providers: [
      {
        providerId: 'platform',
        adapterAction: 'edit_image',
        executionTool: 'image.edit'
      }
    ],
  }
];
